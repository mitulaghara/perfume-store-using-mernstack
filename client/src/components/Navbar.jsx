    ...
    return (
        <>
            <nav className="fixed top-0 left-0 w-full z-50 glass-nav bg-white/95 backdrop-blur-xl border-b border-black/5">
                    ...
                    <div className="flex items-center space-x-4 md:space-x-8">
                        ...
                        {isAuthenticated ? (
                            <div className="relative">
                                ...
                                >
                                    {user?.avatar && !user.avatar.includes('ui-avatars.com') ? (
                                        ...
                                        <UserAvatar name={user?.name} className="w-8 h-8 border border-black text-[10px] font-black" />
                                    )}
                                </button>

                                {showUserMenu && (
                                    <div className="absolute right-0 mt-3 w-56 bg-white border border-black shadow-2xl py-2 z-50">
                                        ...
                                        <Link
                                            to="/profile"
                                            className="flex items-center space-x-3 px-4 py-3 hover:bg-black hover:text-white transition group text-sm font-medium"
                                        ...
                                        {user?.role === 'admin' && (
                                            <Link
                                                to="/admin"
                                                className="flex items-center space-x-3 px-4 py-3 hover:bg-black hover:text-white transition group text-sm font-black border-t border-gray-50 mt-1"
                                                onClick={() => setShowUserMenu(false)}
                                            >
                                        ...
                                        <button
                                            ...
                                            className="flex items-center space-x-3 px-4 py-3 hover:bg-red-600 hover:text-white transition group w-full text-sm font-medium border-t border-gray-50"
                                        >
                            ...
                            <button
                                ...
                                className="bg-black text-white px-6 py-2 text-xs font-black uppercase tracking-[0.2em] hover:bg-white hover:text-black border border-black transition-all"
                            >
                                Login
                            </button>
                        )}
                ...
                {/* Mobile Menu */}
                {showMobileMenu && (
                    <div className="lg:hidden bg-white border-t border-black absolute w-full left-0 shadow-2xl z-40">
                        <div className="flex flex-col p-6 space-y-6 uppercase tracking-widest text-[10px] font-black">
                            <Link to="/" onClick={() => setShowMobileMenu(false)} className="text-black hover:bg-black hover:text-white p-2">Home</Link>
                            <Link to="/shop" onClick={() => setShowMobileMenu(false)} className="text-black hover:bg-black hover:text-white p-2">Shop</Link>
                            <Link to="/orders" onClick={() => setShowMobileMenu(false)} className="text-black hover:bg-black hover:text-white p-2">Orders</Link>
                            {isAuthenticated && (
                                <>
                                    <Link to="/profile" onClick={() => setShowMobileMenu(false)} className="text-black hover:bg-black hover:text-white p-2">My Profile</Link>
                                    {user?.role === 'admin' && (
                                        <Link to="/admin" onClick={() => setShowMobileMenu(false)} className="bg-black text-white p-4 flex items-center gap-4">
                                            <LayoutDashboard className="w-5 h-5" /> ADMIN PANEL
                                        </Link>
                                    )}
                                    <button onClick={() => { logout(); setShowMobileMenu(false); }} className="text-red-600 font-black p-2 text-left mt-4 border-t border-gray-100">Logout</button>
                                </>
                            )}
                        </div>
                    </div>
